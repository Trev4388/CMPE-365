...
def buildTriangles(slice0, slice1):
    ...
    #check all verticies in each slice to determine closest points between two slices
    minDist=0
    for i in slice0.verts:
        for j in slice1.verts:
            dis=math.sqrt((i.coords[0]-j.coords[0])**2 + (i.coords[1]-j.coords[1])**2 + (i.coords[2]-j.coords[2])**2)
            if dis<minDist or minDist==0:
                minDist=dis
                p0=i
                p1=j
    ...
    #find index of two closests verticies
    index0=slice0.verts.index(p0)
    index1=slice1.verts.index(p1)
    #get length of each vertex list
    slice0Len=len(slice0.verts)
    slice1Len=len(slice1.verts)
    # initialize new empty lists for cyclic permutation of each slice
    cicPerm0=[None]*0
    cicPerm1=[None]*0
    #aappend verticies to new lists, starting and ending with closest points to create cyclic permutation
    for i in slice0.verts:
        if(index0==slice0Len):
            index0=0
        cicPerm0.append(slice0.verts[index0])
        index0=index0+1
    cicPerm0.append(cicPerm0[0])
    slice0Len=slice0Len+1

    for i in slice1.verts:
        if(index1==slice1Len):
            index1=0
        cicPerm1.append(slice1.verts[index1])
        index1=index1+1
    cicPerm1.append(cicPerm1[0])
    slice1Len=slice1Len+1
    ...
    #initialize 2d arrays minArea and minDir with appropriate size
    minArea = [[None for i in range(slice0Len)] for j in range(slice1Len)]  # CHANGE THIS

    minDir = [[None for i in range(slice0Len)] for j in range(slice1Len)]  # CHANGE THIS
    ...
    for i in range(slice0Len): #row 0 of minDir
        if (i==0):
            minDir[0][i]='.'
        else:
            minDir[0][i]=Dir.PREV_COL

    for i in range(slice0Len): #row 0 for minArea
        if (i==0):
            minArea[0][i]=0
        else:
            minArea[0][i]=(triangleArea(cicPerm1[0].coords, cicPerm0[i].coords, cicPerm0[i-1].coords) + minArea[0][i-1])
    ...
    # [2 marks]
    #
    for i in range(slice1Len): #col 0 of minDir
        if (i==0):
            minDir[i][0]='.'
        else:
            minDir[i][0]=Dir.PREV_ROW
    for i in range(slice1Len): #col 0 for minArea
        if (i==0):
            minArea[i][0]=0
        else:
            minArea[i][0]=(triangleArea(cicPerm0[0].coords, cicPerm1[i-1].coords, cicPerm1[i].coords)+minArea[i-1][0])
    ...
    # [YOUR CODE HERE]
    #populate arrays with remaining area and direction values
    for i in range(slice1Len-1):
        for j in range(slice0Len-1): # use i+1 and j+1
            minArea[i+1][j+1]=min((minArea[i][j+1]+triangleArea(cicPerm1[i].coords,cicPerm1[i+1].coords,cicPerm0[j+1].coords)),(minArea[i+1][j]+triangleArea(cicPerm0[j+1].coords,cicPerm0[j].coords,cicPerm1[i+1].coords)))
            if (minArea[i+1][j+1]==(minArea[i+1][j]+triangleArea(cicPerm0[j+1].coords,cicPerm0[j].coords,cicPerm1[i+1].coords))):
                minDir[i+1][j+1]=Dir.PREV_COL
            else:
                minDir[i+1][j+1]=Dir.PREV_ROW
    ...
    #initialize index variables for maximum index in each slice
    slice0Idx=slice0Len-1
    slice1Idx=slice1Len-1

    #Work backwards through 2d array to compute minimum-area triangulation between two contours. Chooses appropriate
    #verticies based on direction of shortest path and creates triangle. Appends triangle to triangle list
    while(minDir[slice1Idx][slice0Idx]!='.'):
        if(minDir[slice1Idx][slice0Idx]==Dir.PREV_COL):
            vert1=(cicPerm0[slice0Idx])
            vert2=(cicPerm1[slice1Idx])
            vert3=(cicPerm0[slice0Idx-1])
            verts=[vert1,vert2,vert3]
            tri=Triangle(verts)
            triangles.append(tri)
            slice0Idx=slice0Idx-1

        else:
            vert1 = (cicPerm0[slice0Idx])
            vert2 = (cicPerm1[slice1Idx])
            vert3 = (cicPerm1[slice1Idx-1])
            verts = [vert1, vert2, vert3]
            tri = Triangle(verts)
            triangles.append(tri)
            slice1Idx = slice1Idx-1
